from dagster import asset, MetadataValue, sensor, \
    ...
    PipesSubprocessClient, file_relative_path
...
def read_data(context: AssetExecutionContext, raw_data: DataToFileResource, bronze_data: DataToFileResource, pipes_subprocess_client: PipesSubprocessClient):
    context.log.debug(f"Read download_data_path")
    
    cmd = [shutil.which("python"), file_relative_path(__file__, "external_code_day12.py"), raw_data.path, bronze_data.path]
